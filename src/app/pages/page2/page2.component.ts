...
export class Page2Component implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (task) {
      ...
      const isUrgent = localStorage.getItem('state') === 'true';
      console.log(isUrgent);
      this.isActive = isUrgent;
  ...
  saveTask(): void {
    if (this.isInputEmpty) {
      ...
      this.newTask = this.activeTodo?.content
        ? this.activeTodo.content + ' ' + this.newTask
        : this.newTask;
  ...
  btnCheckActiveModif() {
    ...
    if (state) {
      const task = JSON.parse(state);
      if (task.isUrgent) {
        this.isUrgent = true;
        this.isActive = true; // activate the button
      }
    }
  }
